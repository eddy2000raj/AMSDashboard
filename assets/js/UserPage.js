...
$scope.name = mytext;
function TotalSRs() { return " Select count(this) FROM config.amvis.core.AMVisTicket t LEFT OUTER JOIN config.amvis.core.AMVisAccount acc USING t.AMVisAccount JOIN ariba.user.core.User as AssignedTo using t.AMVisAssignedTo WHERE t.AMVisStatus = 'OPEN' and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+mytext+"' ";}
...
$scope.name = mytext;
function customerUpdate() { return " Select count(this) FROM config.amvis.core.AMVisTicket t LEFT OUTER JOIN config.amvis.core.AMVisAccount acc USING t.AMVisAccount JOIN ariba.user.core.User as AssignedTo using t.AMVisAssignedTo WHERE t.AMVisStatus = 'OPEN' AND t.AMVisSubStatus in ('Customer Update','Research','Screen Sharing Scheduled','Internal Assistance','Internal Update','Internal Review','Initial Review','Implemented in Test') and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+mytext+"'";}
...
$scope.name = mytext;
function onHold() { return " Select count(this) from AMVisTicket t JOIN ariba.user.core.User u USING t.AMVisAssignedTo WHERE AMVisStatus = 'OPEN' AND AMVisSubStatus = 'Customer - Hold' and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+$scope.name+"'";}
$scope.onHold = onHold();
               $http({
				method : "POST",
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
$scope.name = mytext;
function PCI() { return " Select count(this) from AMVisTicket t JOIN ariba.user.core.User u USING t.AMVisAssignedTo WHERE AMVisStatus = 'OPEN' AND AMVisSubStatus = 'Pending Customer Input' and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+$scope.name+"'";}
$scope.PCI = PCI();
               $http({
				method : "POST",
				headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
$scope.name = mytext;
function ResolvedConfirm() { return " Select count(this) FROM config.amvis.core.AMVisTicket t LEFT OUTER JOIN config.amvis.core.AMVisAccount acc USING t.AMVisAccount JOIN ariba.user.core.User as AssignedTo using t.AMVisAssignedTo WHERE t.AMVisStatus = 'OPEN' AND AMVisSubStatus in ('Resolved - Confirm','Pending Customer Confirmation') and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+mytext+"'";}
...
$scope.name = mytext;
function TSSTickets() { return " Select count(this) from AMVisTicket t JOIN ariba.user.core.User u USING t.AMVisAssignedTo WHERE AMVisStatus = 'OPEN' AND AMVisSubStatus = 'TSS'  and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+$scope.name+"'";}
...
function customerUpdate(){ return " Select t.AMVisSRNumber as \"SR Number\",acc.AMVisShortName AS \"Account\",SUBSTRING(t.AMVisDescription,0,40) AS \"Description\",SUBSTRING(t.AMVisPriority,0,3) AS \"Priority\",Round((CurrentDate()-t.AMVisDateUpdated)) as \"PendingDays\",t.AMVisSubStatus as \"Status\" FROM config.amvis.core.AMVisTicket t LEFT OUTER JOIN config.amvis.core.AMVisAccount acc USING t.AMVisAccount JOIN ariba.user.core.User as AssignedTo using t.AMVisAssignedTo WHERE t.AMVisStatus = 'OPEN' AND AMVisSubStatus IN ('Customer Update','Research','Screen Sharing Scheduled','Internal Assistance','Internal Update','Internal Review','Initial Review','Implemented in Test') and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+mytext+"' ORDER BY t.AMVisDateUpdated ASC ";}
...
var data = new FormData();
function PendingCustomerInputTable(){ return " Select t.AMVisSRNumber as \"SR Number\",acc.AMVisShortName AS \"Account\",SUBSTRING(t.AMVisDescription,0,40) AS \"Description\",SUBSTRING(t.AMVisPriority,0,3) AS \"Priority\",Round((CurrentDate()-t.AMVisDateUpdated)) as \"PendingDays\" FROM config.amvis.core.AMVisTicket t LEFT OUTER JOIN config.amvis.core.AMVisAccount acc USING t.AMVisAccount JOIN ariba.user.core.User as AssignedTo using t.AMVisAssignedTo WHERE t.AMVisStatus = 'OPEN' AND AMVisSubStatus = 'Pending Customer Input' and t.AMVisIsAMSTicket!=false AND AMVisAssignedTo.Name.PrimaryString ='"+mytext+"' ORDER BY t.AMVisDateUpdated ASC ";}
...
var data = new FormData();
data.append("query", "SELECT distinct CASE acc.ShortName when null then acc.UniqueName else acc.ShortName end as Customer,acc.AMVisSOProjectId as ProjectId,ROUND(SUM(AMVisDuration/60))  FROM amvis.analytics.fact.AMVisTicketEffort te JOIN ariba.analytics.dimension.Time t USING te.AMVisStartDate JOIN amvis.analytics.dimension.AMVisAccount acc USING te.AMVisAccount JOIN ariba.analytics.dimension.UserData ud USING te.AMVisUser WHERE CurrentDate() - t.Day < 8 and ud.UserName='"+mytext+"'  GROUP BY acc.ShortName,acc.UniqueName,acc.AMVisSOProjectId");
data.append("useSQL", "");
data.append("type", "table");
...
function getCurrentWeekDateObj(){
            ...
            range.firstDate=moment(firstday).format('YYYY-MM-DD HH:MM:SS')+" PDT";
            range.lastDate=moment(lastday).format('YYYY-MM-DD HH:MM:SS')+" PDT";

            return range;
}
       
console.log(getCurrentWeekDateObj().firstDate);
console.log(getCurrentWeekDateObj().lastDate);
